...
exports.getBrasseur = async (req, res) => {
  const listbrewer = await queryAsync(
    "SELECT nameBrass, nameTown, nameWeb, nameFacebook FROM brewersfrench WHERE nameCp LIKE '85%' limit 5"
  )
...
exports.getBrasseursAPI = async (req, res) => {
  const listbrewer = await queryAsync(
    'SELECT brewer_id as id, nameBrass, address, nameWeb, nameFacebook, nameTown, ville_departement, ville_code_postal, ville_longitude_deg as lng, ville_latitude_deg as lat  FROM `brewersfrench`INNER JOIN villes_france_free ON nameTown = ville_nom '
  )
...
exports.getDepartementAPI = async (req, res) => {
  const listdepartement = await queryAsync(
    'SELECT `departement_id`, `departement_code`, `departement_nom`, `departement_nom_uppercase` FROM `departement`  ORDER BY   departement_id '
  )
...
exports.getBrasseursDepartementAPI = async (req, res) => {
  ...
  const lienbrasseurdepart = await queryAsync(
    "SELECT `departement_nom`, GROUP_CONCAT(`nameBrass` SEPARATOR ' , ') AS nameBrass FROM `brewersfrench` INNER JOIN `departement` ON `departement_code` = `nameCode` GROUP BY `departement_nom`"
  )
